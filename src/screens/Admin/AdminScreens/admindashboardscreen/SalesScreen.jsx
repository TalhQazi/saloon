...
const loadAdminCartCount = async setCartCount => {
  try {
    ...
    console.error('[SalesScreen] Failed to load admin cart count:', err);
    setCartCount(0);
  }
};
...
const CACHE_KEY = 'salesScreenCache';
...
const SalesScreen = ({ navigation }) => {
  ...
  // Responsive column widths state
  const [responsiveWidths, setResponsiveWidths] = useState(COL_WIDTHS);

  useEffect(() => {
    // Calculate responsive widths based on screen size
    const calculated = getResponsiveColWidths(width);
    setResponsiveWidths(calculated);
  }, [width]);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setUserName(adminData?.name || 'Guest');

      const image = adminData?.profilePicture;
  ...
  useEffect(() => {
    ...
    const sub = DeviceEventEmitter.addListener(ADMIN_CART_UPDATED_EVENT, () => {
      loadAdminCartCount(setCartCount);
    });
    return () => sub.remove();
  }, []);
  ...
  const loadSales = useCallback(
    ...
    [authenticatedAdmin?.token, extractVisitsFromClient],
  );
  ...
  }, [authenticatedAdmin?.token, loadSales]);
