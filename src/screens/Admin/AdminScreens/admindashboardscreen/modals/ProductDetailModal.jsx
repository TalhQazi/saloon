...
const ProductDetailModal = ({ visible, onClose, product }) => {
    ...
    // Resolve the list of product detail items from either productDetails or subProducts
    const detailItems =
        product.productDetails && product.productDetails.length
                ...
                <View style={styles.modalContainer}>
                    ...
                    <ScrollView contentContainerStyle={styles.scroll}>
                        ...
                        <Text style={styles.detailText}>
                            {product.name || product.productName || product.title || 'N/A'}
                        </Text>

                        {detailItems && detailItems.length > 0 ? ( // Added conditional rendering for sub-section
                            <View>
                                <Text style={styles.detailLabel}>Product Items:</Text>
                                {detailItems.map((detail, index) => (
                                    <View key={detail.id || index.toString()} style={styles.subServiceItem}>
                                        { (detail.image || detail.productDetailImage) &&
                                          getSubImageSource(detail.image || detail.productDetailImage) && (
                                            <Image
                                                source={getSubImageSource(detail.image || detail.productDetailImage)}
                                        ...
                                        <View style={styles.subServiceTextContainer}>
                                            ...
                                            <Text style={styles.subServiceText}>
                                                Name: {detail.name || detail.productDetailName || 'N/A'}
                                            </Text>
                                            <Text style={styles.subServiceText}>
                                                Price: PKR {detail.price != null ? detail.price : '0'}
                                            </Text>
                                            <Text style={styles.subServiceText}>Time: {detail.time}</Text>
                                            {detail.description && (
                                                <Text style={styles.subServiceText}>{detail.description}</Text>
                                            )}
                                        </View>
                                    </View>
                                ))}
