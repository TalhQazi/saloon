...
const AttendanceFaceRecognitionScreen = ({ route, navigation }) => {
  ...
  // Handler for when the modal's save button is pressed
  // THIS IS THE KEY CHANGE
  const handleRequestSave = data => {
    setShowRequestModal(false); // Close the modal
    // Navigate to the new confirmation screen and pass the request details
    navigation.navigate('AttendanceConfirmationRecognitionScreen', {
      employee,
      requestDetails: data,
    });
  };
  ...
  const handleRequestModalClose = () => {
    ...
    setStatus('Attendance request cancelled.');
  ...
  const startFaceRecognitionProcess = async () => {
    ...
    try {
      ...
      if (result.success) {
        setStatus('Face recognized! Please add attendance details.');
    ...
    <View style={styles.centeredView}>
      ...
      >
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>Attendance Face Registration</Text>
          <TouchableOpacity
            onPress={() => navigation.goBack()}
...
export default AttendanceFaceRecognitionScreen;
