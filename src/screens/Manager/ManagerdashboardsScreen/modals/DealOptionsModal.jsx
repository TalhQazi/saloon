...
const DealOptionsModal = ({ visible, onClose, onSelectOption, position, isHidden = false }) => { // <<< FIX: Added = false
    ...
    return (
        ...
        >
            <TouchableOpacity style={optionsStyles.overlay} onPress={onClose} activeOpacity={1}>
                <View style={[optionsStyles.optionsContainer, { top: position.top, left: position.left }]}>
                    {options.map((option, index) => (
                        <TouchableOpacity
                            // The key is now safe because isHidden will always be a boolean
                            key={option.value + (option.label === 'Show' || option.label === 'Hide' ? isHidden.toString() : '')}
