      ...
      if (response.data.success) {
        ...
        const mappedVisits = fetchedVisits.map(visit => {
          ...
          const mappedVisit = {
            ...
            // ✅ Core bill data - same structure as CartServiceScreen creates
            services: services,
            subtotal: visitData.subtotal || visit.subtotal || servicesSubtotal,
            discount: visitData.discount || visit.discount || 0,
            notes: visitData.notes || visit.notes || '-',
            beautician: visitData.beautician || visit.beautician || '-',

            // ✅ Client info - same as PrintBillModal expects
            clientName: visitData.clientName || client?.name || 'Guest',
            phoneNumber: visitData.phoneNumber || client?.phoneNumber || '-',

            // ✅ Bill metadata - same as PrintBillModal expects
            billNumber:
              visitData.billNumber ||
              visit.billNumber ||
              visit.visitId ||
              `BILL-${Date.now()}`,
            date: visitData.date || visit.date || new Date().toISOString(),

            // ✅ Total calculation - same as CartServiceScreen
            totalBill:
              visitData.totalBill ||
              visit.totalBill ||
              servicesSubtotal - (visitData.discount || visit.discount || 0),
