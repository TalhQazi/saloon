...
import { getManagerCartItems, removeManagerCartItem } from '../../../utils/managerCart';

const { width, height } = Dimensions.get('window');

const ManagerCartScreen = () => {
  const navigation = useNavigation();
  const [items, setItems] = useState([]);

  const loadItems = useCallback(async () => {
    const data = await getManagerCartItems();
    setItems(data);
  }, []);
  ...
  const handleRemove = async id => {
    const updated = await removeManagerCartItem(id);
    setItems(updated);
  };
...
export default ManagerCartScreen;
